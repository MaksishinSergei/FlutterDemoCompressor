...
class _HomePageState extends State<HomePage> {
    ...
    try {
      ...
      if (image != null && mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => MediaViewScreen(filePath: image.path, videoFile: null),
          ),
        );
        setState(() {});
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Ошибка при открытии галереи: ${e.toString()}')),
        );
      }
    }
  }
  ...
  Future<void> _openGalleryVideo() async {
    ...
    try {
      final video = await MediaPicker.pickVideo();
      if (video != null && mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => MediaViewScreen(filePath: video.path, videoFile: File(video.path)),
          ),
        );
        setState(() {});
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Сжатие медиа", style: TextStyle(color: appBarTextColor)),
        centerTitle: true,
